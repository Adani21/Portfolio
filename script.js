    ...
    function createFloatingShapes() {
      ...
      shapes.forEach(shape => {
        ...
        if (shape.classList.contains('shape-circle')) {
          body = Bodies.circle(startX, startY, 40, { restitution: 0.8, friction: 0.001, frictionAir: 0.01 });
        } else if (shape.classList.contains('shape-square')) {
          body = Bodies.rectangle(startX, startY, 70, 70, { restitution: 0.8, friction: 0.001, frictionAir: 0.01 });
        } else if (shape.classList.contains('shape-triangle')) {
          const vertices = [{ x: 0, y: -35 }, { x: -35, y: 35 }, { x: 35, y: 35 }];
          body = Bodies.fromVertices(startX, startY, vertices, { restitution: 0.8, friction: 0.001, frictionAir: 0.01 });
        }

        Body.setVelocity(body, { x: gsap.utils.random(-1, 1), y: gsap.utils.random(-1, 1) });
      ...
      Events.on(engine, 'beforeUpdate', () => {
        bodies.forEach(({ body }) => {
          ...
          if (distance < maxDistance && distance > 10) {
            const force = { x: (dx / distance) * attractionStrength * body.mass, y: (dy / distance) * attractionStrength * body.mass };
            Body.applyForce(body, body.position, force);
          }
        });
      });
